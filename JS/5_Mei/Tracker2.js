...
for (let itemIndex = 1; itemIndex <= 8; itemIndex++) {
  ...
  // ⭐ Unieke MEI key voor labels
  labelInput.dataset.key = `mei_tracker_label_${itemIndex}`;

  // 🔥 laad label uit Firebase
  db.collection("mei_tracker_labels").doc(labelInput.dataset.key).get().then(doc => {
    if (doc.exists) {
  ...
  labelInput.addEventListener("input", () => {
    db.collection("mei_tracker_labels").doc(labelInput.dataset.key).set({
...
function createDayCell(itemIndex, dayNumber) {
  ...
  // ⭐ Unieke MEI key voor dag-vakjes
  const key = `mei_tracker_item${itemIndex}_day${dayNumber}`;

  // 🔥 laad kleur uit Firebase
  db.collection("mei_tracker_days").doc(key).get().then(doc => {
  ...
  cell.addEventListener("click", () => {
    ...
    db.collection("mei_tracker_days").doc(key).set({
      active: active
    });
  });
