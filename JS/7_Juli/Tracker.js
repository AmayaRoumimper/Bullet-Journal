...
for (let layer = 0; layer < LAYERS - 1; layer++) {
    ...
    for (let d = 0; d < DAYS; d++) {
        ...
        // ⭐ Unieke JULI key
        const key = `jul_cell_${layer}_${d}`;

        // 🔥 Laden uit Firebase
        db.collection("jul_tracker_cells").doc(key).get().then(doc => {
            if (doc.exists) {
        ...
        cell.addEventListener("click", () => {
            ...
            if (layer <= 2) {
                ...
                db.collection("jul_tracker_cells").doc(key).set({
                    color: newColor
                });
...
woordVakken.forEach((vak) => {
    const key = `jul_${vak.dataset.key}`;

    // 🔥 Laden uit Firebase
    db.collection("jul_tracker_words").doc(key).get().then(doc => {
        if (doc.exists) {
    ...
    vak.addEventListener("input", () => {
        db.collection("jul_tracker_words").doc(key).set({
            value: vak.value
        });
    });
});
