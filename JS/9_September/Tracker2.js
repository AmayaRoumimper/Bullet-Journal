...
for (let itemIndex = 1; itemIndex <= 8; itemIndex++) {
  ...
  // ⭐ Unieke SEPTEMBER key voor labels
  labelInput.dataset.key = `sep_tracker_label_${itemIndex}`;

  // 🔥 laad label uit Firebase
  db.collection("sep_tracker_labels").doc(labelInput.dataset.key).get().then(doc => {
    if (doc.exists) {
  ...
  labelInput.addEventListener("input", () => {
    db.collection("sep_tracker_labels").doc(labelInput.dataset.key).set({
...
function createDayCell(itemIndex, dayNumber) {
  ...
  // ⭐ Unieke SEPTEMBER key voor dag-vakjes
  const key = `sep_tracker_item${itemIndex}_day${dayNumber}`;

  // 🔥 laad kleur uit Firebase
  db.collection("sep_tracker_days").doc(key).get().then(doc => {
  ...
  cell.addEventListener("click", () => {
    ...
    db.collection("sep_tracker_days").doc(key).set({
      active: active
    });
  });
