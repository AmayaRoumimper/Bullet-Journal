...
for (let layer = 0; layer < LAYERS - 1; layer++) {
    ...
    for (let d = 0; d < DAYS; d++) {
        ...
        // ⭐ Unieke SEPTEMBER key
        const key = `sep_cell_${layer}_${d}`;

        // 🔥 Laden uit Firebase
        db.collection("sep_tracker_cells").doc(key).get().then(doc => {
            if (doc.exists) {
        ...
        cell.addEventListener("click", () => {
            ...
            if (layer <= 2) {
                ...
                db.collection("sep_tracker_cells").doc(key).set({
                    color: newColor
                });
...
woordVakken.forEach((vak) => {
    ...
    // 🔥 Laden uit Firebase
    db.collection("sep_tracker_words").doc(key).get().then(doc => {
        if (doc.exists) {
    ...
    vak.addEventListener("input", () => {
        db.collection("sep_tracker_words").doc(key).set({
            value: vak.value
        });
    });
});
