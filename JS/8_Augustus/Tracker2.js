...
for (let itemIndex = 1; itemIndex <= 8; itemIndex++) {
  ...
  // ⭐ Unieke AUGUSTUS key voor labels
  labelInput.dataset.key = `aug_tracker_label_${itemIndex}`;

  // 🔥 laad label uit Firebase
  db.collection("aug_tracker_labels").doc(labelInput.dataset.key).get().then(doc => {
    if (doc.exists) {
  ...
  labelInput.addEventListener("input", () => {
    db.collection("aug_tracker_labels").doc(labelInput.dataset.key).set({
...
function createDayCell(itemIndex, dayNumber) {
  ...
  // ⭐ Unieke AUGUSTUS key voor dag-vakjes
  const key = `aug_tracker_item${itemIndex}_day${dayNumber}`;

  // 🔥 laad kleur uit Firebase
  db.collection("aug_tracker_days").doc(key).get().then(doc => {
  ...
  cell.addEventListener("click", () => {
    ...
    db.collection("aug_tracker_days").doc(key).set({
      active: active
    });
  });
