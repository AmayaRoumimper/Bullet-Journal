...
for (let itemIndex = 1; itemIndex <= 8; itemIndex++) {
  ...
  // ⭐ Unieke DECEMBER key voor labels
  labelInput.dataset.key = `dec_tracker_label_${itemIndex}`;

  // 🔥 laad label uit Firebase
  db.collection("dec_tracker_labels").doc(labelInput.dataset.key).get().then(doc => {
    if (doc.exists) {
  ...
  labelInput.addEventListener("input", () => {
    db.collection("dec_tracker_labels").doc(labelInput.dataset.key).set({
...
function createDayCell(itemIndex, dayNumber) {
  ...
  // ⭐ Unieke DECEMBER key voor dag-vakjes
  const key = `dec_tracker_item${itemIndex}_day${dayNumber}`;

  // 🔥 laad kleur uit Firebase
  db.collection("dec_tracker_days").doc(key).get().then(doc => {
  ...
  cell.addEventListener("click", () => {
    ...
    db.collection("dec_tracker_days").doc(key).set({
      active: active
    });
  });
