...
for (let itemIndex = 1; itemIndex <= 8; itemIndex++) {
  ...
  // ⭐ Unieke MAART key voor labels
  labelInput.dataset.key = `maa_tracker_label_${itemIndex}`;

  // 🔥 laad label uit Firebase
  db.collection("maa_tracker_labels").doc(labelInput.dataset.key).get().then(doc => {
    if (doc.exists) {
  ...
  labelInput.addEventListener("input", () => {
    db.collection("maa_tracker_labels").doc(labelInput.dataset.key).set({
...
function createDayCell(itemIndex, dayNumber) {
  ...
  // ⭐ Unieke MAART key voor dag-vakjes
  const key = `maa_tracker_item${itemIndex}_day${dayNumber}`;

  // 🔥 laad kleur uit Firebase
  db.collection("maa_tracker_days").doc(key).get().then(doc => {
  ...
  cell.addEventListener("click", () => {
    ...
    db.collection("maa_tracker_days").doc(key).set({
      active: active
    });
  });
